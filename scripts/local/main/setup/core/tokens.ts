...
export default async (state: ContractsState): Promise<ContractsState> => {
    ...
    await Promise.all(state.signers.map(async (signer) => wEth.mint(signer.address, ethers.utils.parseEther("10000"))));
    ...
    for (let i = 0; i < state.NUM_TOKENS; i++) {
        ...
        await Promise.all(
            state.signers.map(async (signer) => token.mint(signer.address, ethers.utils.parseEther("10000")))
        );
